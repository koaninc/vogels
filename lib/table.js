...
internals.waitForActive = function (table, callback) {
  table.describeTable(function (err, data) {
    if (err) {
      return callback(err);
    }
    var isTableReady = data.Table.TableStatus === 'ACTIVE';
    var isEveryIndexReady = data.Table.GlobalSecondaryIndexes.every(function (idx) {
      return idx.IndexStatus === 'ACTIVE';
    });

    if (isTableReady && isEveryIndexReady) {
      return callback(null);
    }

    setTimeout(internals.waitForActive.bind(null, table, callback), 1000);
  });
};
...
internals.syncIndexes = function (table, callback) {
  ...
  table.describeTable(function (err, data) {
    ...
    var maxIndexCreationsAtaTime = 1;
    async.mapLimit(missing, maxIndexCreationsAtaTime, function (params, callback) {
      ...
      table.sendRequest('updateTable', updateParams, function (err, data) {
        if (err) {
          return callback(err);
        }
        internals.waitForActive(table, callback);
      });
    }, callback);
  });
};
